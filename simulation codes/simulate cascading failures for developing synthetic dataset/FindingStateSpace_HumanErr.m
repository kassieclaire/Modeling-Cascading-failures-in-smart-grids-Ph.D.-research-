function [States] = FindingStateSpace_HumanErr(CaseName, Iterations, InitialFailures, LoadGenerationRatio, LoadShedConstant, EstimationError, MaxFailedCapacity)
    define_constants;
    caseName = CaseName;
    NumIt = Iterations;
    %= InitialFailures; 
    DGRatio = LoadGenerationRatio;
    ...
    %%
    %Confusing section -- ask about this
    ...
    for s=1:NumIt
        ...
        States(StateCounter,15)=Demand; % Time of the failure event -- is this set?
    ...
    % For 118 case
    if strcmp(caseName, 'case118')
        NumberOfLines = 186;
        fprintf('Using IEEE 118 case, the number of lines is %f \n',NumberOfLines);
    elseif strcmp(caseName, 'case300')
        NumberOfLines = 411;
        fprintf('Using IEEE 300 case, the number of lines is %f \n',NumberOfLines);
...
%     end
    ...
    %set(gca,'fontname','times', 'fontsize', 24)  % Set font to times 24

%     figure (2)
%     plot(3:NumberOfLines, cascade_stop(3:NumberOfLines))
%     xlabel('Number of failed transmission lines', 'FontSize', 24)
%     ylabel('Cascade-stop probability', 'FontSize', 24)
%     %Plot # of failed transmission lines vs time
%     figure(3)
%     plot(States(:,15), States(:,1));
%     xlabel('Time (seconds)', 'FontSize', 24)
%     ylabel('Number of failed transmission lines', 'FontSize', 24)
%     %Plot hop distance vs time
%     figure(4)
%     plot(States(:,15), States(:,1));
%     xlabel('Time (seconds)', 'FontSize', 24)
%     ylabel('Average Hop Distance', 'FontSize', 24)
end